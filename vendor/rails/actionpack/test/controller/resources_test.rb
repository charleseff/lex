...
class ResourcesTest < Test::Unit::TestCase
  ...
  def test_should_arrange_actions
    resources = ActionController::Resources::Resource.new(:messages,
      :collection => { :rss => :get, :reorder => :post, :csv => :post },
      :member     => { :rss => :get, :atom => :get, :upload => :post, :fix => :post },
      :new        => { :preview => :get, :draft => :get })

    assert_resource_methods [:rss],                   resources, :collection, :get
    assert_resource_methods [:csv, :reorder],         resources, :collection, :post
    assert_resource_methods [:edit, :rss, :atom],     resources, :member,     :get
    assert_resource_methods [:upload, :fix],          resources, :member,     :post
    assert_resource_methods [:new, :preview, :draft], resources, :new,        :get
  end

  def test_should_resource_controller_name_equal_resource_name_by_default
    resources = ActionController::Resources::Resource.new(:messages, {})
    assert_equal 'messages', resources.controller
  end

  def test_should_resource_controller_name_equal_controller_option
    resources = ActionController::Resources::Resource.new(:messages, :controller => 'posts')
    assert_equal 'posts', resources.controller
  end

  def test_should_all_singleton_paths_be_the_same
    [ :path, :nesting_path_prefix, :member_path ].each do |method|
      resources = ActionController::Resources::SingletonResource.new(:messages, :path_prefix => 'admin')
      assert_equal 'admin/messages', resources.send(method)
    end
  end
  ...
  def test_should_nest_resources_in_singleton_resource
    with_routing do |set|
      set.draw do |map|
        map.resources :account do |account|
  ...
  def test_should_nest_resources_in_singleton_resource_with_path_prefix
    with_routing do |set|
      set.draw do |map|
        map.resources(:account, :path_prefix => ':site_id') do |account|
          account.resources :messages
        end
      end
    ...
    with_routing do |set|
      set.draw do |map|
        map.resources :threads do |thread|
          thread.resources :admin, :controller => 'admin'
        end
      end
  ...
  def test_resource_action_separator
    with_routing do |set|
      set.draw do |map|
        ...
        map.resources :account, :member => {:login => :get}, :new => {:preview => :any}, :name_prefix => 'admin_', :path_prefix => '/admin'
      end
  ...
  protected
    ...
    def assert_resource_methods(expected, resources, action_method, method)
      assert_equal expected.length, resources.send("#{action_method}_methods")[method].size, "#{resources.send("#{action_method}_methods")[method].inspect}"
      expected.each do |action|
        assert resources.send("#{action_method}_methods")[method].include?(action),
          "#{method} not in #{action_method} methods: #{resources.send("#{action_method}_methods")[method].inspect}"
      end
    end
