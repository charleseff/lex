...
module ActiveResource
  ...
  class Base
    ...
    class << self
      ...
      private
        ...
        def instantiate_record(record, prefix_options = {})
          returning new(record) do |resources|
            resources.prefix_options = prefix_options
          end
        end
    ...
    #   # => That Company
    def dup
      returning self.class.new do |resources|
    ...
    def load(attributes)
      ...
      attributes.each do |key, value|
        @attributes[key.to_s] =
          case value
            when Array
              resources = find_or_create_resource_for_collection(key)
              value.map { |attrs| resources.new(attrs) }
            when Hash
              resources = find_or_create_resource_for(key)
              resources.new(value)
            else
    ...
    private
      ...
      def find_or_create_resource_for(name)
        ...
        resources = self.class.const_set(resource_name, Class.new(ActiveResource::Base))
        resources.prefix = self.class.prefix
        resources.site   = self.class.site
        resources
      end
